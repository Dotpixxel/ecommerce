...
export function SignupForm({
	...
	const emailForm = useForm({
		...
		validators: { onChange: signupEmailSchema },
		onSubmit: async ({ value }) => {
			try {
				await sendOtp({
					email: value.email,
					type: "sign-in", // Creates account if doesn't exist
				});
	...
	const otpForm = useForm({
		...
		onSubmit: async ({ value }) => {
			try {
				...
				// Optional: Update the user's name after successful OTP sign in / account creation
				try {
